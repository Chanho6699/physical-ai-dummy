...
class StagedStepResult:
    ...
    step_error: str | None  # 관측/추론/adapter 단계 실패 시 사유
...
class StagedRealRolloutRunner:
    ...
    def _run_one_step(self, i: int) -> tuple[StagedStepResult, bool, str | None]:
        def _early_stop(reason: str, *, before: dict | None = None, raw: dict | None = None) -> tuple[StagedStepResult, bool, str]:
            return (
                StagedStepResult(
        ...
        predict: PredictResult = self._vla_client.predict(
            session_id=f"staged-{i}", task=self._task, sequence=i, state=before_state, images=built.images
        )
        if not predict.ok:
            return _early_stop(f"VLA {predict.error_kind} 오류: {predict.error_message}", before=before_state)
