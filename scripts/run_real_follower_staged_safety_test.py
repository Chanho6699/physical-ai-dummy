...
## 실행 예시 (실제 로봇 앞, 반드시 로봇을 직접 보면서)
    ...
    # 1) 먼저 dry-run으로 계획만 확인 (하드웨어 접근 없음)
    ...
    # 2) stage 1 (single accepted action) - 실제 실행
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument("--inference-device", default=None)
...
def print_banner(*, stage: int, max_steps: int, checkpoint: Path, task: str, dry_run: bool) -> None:
    print("=" * 70)
...
def main(argv: list[str] | None = None) -> int:
    ...
    print_banner(stage=args.stage, max_steps=max_steps, checkpoint=args.checkpoint, task=args.task, dry_run=args.dry_run)

    if args.dry_run:
        print("\n[dry-run] 하드웨어에 전혀 접근하지 않았습니다. 위 계획이 맞으면 --dry-run 없이 다시 실행하세요.")
    ...
    try:
        ...
        print(f"[준비] SmolVLA checkpoint 로딩 완료: {args.checkpoint}")
