#!/usr/bin/env python3
"""Assemble reports/combined65_reweight_new3_old1_v1/ - the 6-way comparison (V2 baseline /
combined65 baseline / combined65 early-weight / reinforcement30-only / combined65 reweight 2:1 /
combined65 reweight 3:1) for the dataset-reweighting ablation, ratio step 2 (old35:reinforcement30
= 1:3). Read-only aggregation of already-completed run artifacts - no training/eval performed here.
"""
...
OUT_DIR = PROJECT_ROOT / "reports" / "combined65_reweight_new3_old1_v1"
...
def main() -> int:
    ...
    rw3_sweeps = {s: load(OUT_DIR / f"first_action_seed_sweep_{s}" / "seed_sweep.json") for s in STEPS}
    ...
    rw2_train_log = parse_training_log(Path("/tmp/reweight_new2_old1_train.log"))
    rw3_train_log = parse_training_log(Path("/tmp/reweight_new3_old1_train.log"))
    ...
    temporal_rw2 = {s: load(REPORTS / "combined65_reweight_new2_old1_v1" / "temporal_chunk_error" / f"reweight_{s}" / "temporal_chunk_error.json") for s in STEPS}
    temporal_rw3 = {s: load(OUT_DIR / "temporal_chunk_error" / f"reweight_{s}" / "temporal_chunk_error.json") for s in STEPS}

    # ---- checkpoint_metrics.csv ----
    def by_step(offline):
        return {r["checkpoint_step"]: r for r in offline["rows"]}

    v2_off, c65_off, ew_off, r30_off, rw2_off, rw3_off = (
        by_step(x) for x in (v2_offline, c65_offline, ew_offline, r30_offline, rw2_offline, rw3_offline)
    )
    ...
    rw2_tr = {r["checkpoint_step"]: r for r in rw2_train_log["checkpoint_rows"]}
    rw3_tr = {r["checkpoint_step"]: r for r in rw3_train_log["checkpoint_rows"]}
    ...
    for s in STEPS:
        for exp, tr, off, insample in [
            ...
            ("combined65_reweight_new2_old1_v1", rw2_tr.get(s), rw2_off.get(s), rw2_ins_by.get(s)),
            ("combined65_reweight_new3_old1_v1", rw3_tr.get(s), rw3_off.get(s), rw3_ins_by.get(s)),
        ]:
    ...
    for s in STEPS:
        add_sweep_rows("combined65_reweight_new2_old1_v1", rw2_sweeps[s], s)
    for s in STEPS:
        add_sweep_rows("combined65_reweight_new3_old1_v1", rw3_sweeps[s], s)
    ...
    for label, temporal in [
        ...
        ("combined65_reweight_new2_old1_v1", temporal_rw2),
        ("combined65_reweight_new3_old1_v1", temporal_rw3),
    ]:
        for s in STEPS:
            d = temporal[s]
    ...
    summary = {
        ...
        "raw": {
            "rw3_train_log_info": {k: v for k, v in rw3_train_log.items()},
            "rw3_offline_eval": rw3_offline,
            "rw3_in_sample_fit": rw3_insample,
            "rw3_sweeps": rw3_sweeps,
            "rw3_temporal": {s: temporal_rw3[s] for s in STEPS},
        },
    }
