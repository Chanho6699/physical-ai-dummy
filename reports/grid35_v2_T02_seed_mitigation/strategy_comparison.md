...
- Nearest-training-demo match: episode 34, frame 0
...
|---|---:|---:|---:|---:|---:|---:|---|---|---:|---:|---:|
| single_random_sample | 55.0% | 0.500 | 5.939 | 5.282 | 9.913 | 13.098 | +5.159±3.185 (15.780) | -2.278±2.341 (10.968) | 1.00 | 1 | 45.0% |
| 3_sample_median | 57.9% | 0.421 | 5.066 | 4.674 | 8.730 | 9.453 | +4.992±1.623 (7.256) | -2.358±1.223 (6.531) | 3.00 | 3 | 42.1% |
| 5_sample_median | 60.4% | 0.396 | 4.832 | 4.535 | 6.976 | 9.009 | +4.866±1.195 (6.136) | -2.420±0.837 (4.912) | 5.00 | 5 | 39.6% |
| safety_pass_resampling_max3 | 92.7% | 0.082 | 4.190 | 3.804 | 7.287 | 7.287 | +3.163±1.950 (7.745) | -1.364±1.983 (8.966) | 1.64 | 3 | 7.3% |
| safety_pass_resampling_max5 | 99.2% | 0.009 | 4.082 | 3.804 | 7.287 | 7.287 | +3.022±1.939 (7.745) | -1.231±1.918 (8.966) | 1.74 | 5 | 0.8% |

## Baseline-only: fix one good seed (NOT evaluated as a deployment candidate)

Selection rule: min GT L2 among clamp-free seeds in the 0..19 sweep (retrospective, GT-informed - NOT reproducible at deploy time). Chosen seed: **6**.
...
| GT L2 (deg) | 2.340 |
| shoulder_lift (deg) | +2.014 |
| elbow_flex (deg) | -1.698 |
...
- single-sample mean GT L2: 5.939 deg
- 3-median mean GT L2: 5.066 deg (improves)
- 5-median mean GT L2: 4.832 deg (improves)
...
> seed 19 is clamp-free (0 joints clamp) but has GT L2=7.287 deg, worse than 4/9 clamped seeds. Clamp-free selection (the only signal available at real deploy time, since GT is unknown) does not guarantee low GT error - it only guarantees the action was not truncated by the Safety Gate.

- clamp-free seeds [6, 2, 11, 12, 18, 3, 13, 9, 8, 4, 19], GT L2 values [2.34, 3.101, 3.186, 3.671, 3.796, 3.804, 3.815, 3.941, 4.968, 5.016, 7.287] (mean 4.084)
- clamped seeds mean GT L2: 8.207
...
**Primary recommendation: safety-pass resampling, cap=5** (clamp-free rate 99.2%, failure rate 0.8%, avg 1.74 inference calls, GT L2 mean 4.082 deg). It directly targets the deployment failure mode this sweep was run to investigate (Safety Gate truncating first-action joints), it is the only strategy of the four whose stopping rule is deploy-time-realistic (uses only clamp status, never GT), and its inference cost is adaptive (only pays for extra calls when the first draw actually clamps) rather than fixed.
